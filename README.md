...
The same opaque source ID and semantic kind are cooled down across the prior three memos, regardless of whether the source came through Brain or Lily. A cooled source is not reused merely to avoid a pending memo. Brain's narrow adjacent-note window remains available to prioritize a just-written relationship letter, while the broader safe reducer can use any causally available Brain thought. Personal context cannot alter the verdict, evidence, action, forecast, chart geometry, or public API shape.
