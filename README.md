...
A recent Brain-generated note may contribute one short, pre-approved sentence of authentic boyfriend connection to a timely memo when it contains either an explicit relationship-safe letter or a safe, strongly first-person signal of Alan's ordinary unfiltered yapping. Brain's generated-note metadata is an eligibility shape, not cryptographic identity proof, so it never authorizes raw-text exposure or factual claims beyond the approved semantic cue. Uploads, transcripts, quoted or third-party material, sensitive generic yaps, and unrelated files are rejected. The server immediately reduces the source to fixed copy and never sends, persists, logs, or exposes the raw entry. Sensitive details, diagnoses, sexual material, breakup fears, conditional affection, and weight-linked relationship claims are ineligible. One Brain source can appear in only one memo and cannot alter the verdict, evidence, action, forecast, chart geometry, or public API shape. Because Brain processing can finish after the weigh-in request, Lily accepts only a bounded six-hour lookback and five-minute post-weight indexing grace, retries across that full window on the server, quietly refreshes the visible memo in the browser, replaces an earlier provisional cue when a newer adjacent entry arrives, and repairs missed current context during authenticated weight reads. Brain downtime leaves the ordinary evidence-first coach path unchanged.
