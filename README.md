...
The primary card shows one direct `1 wk · 1 mo · 1 yr` line and a short pattern-specific coach read. The coach counts the real streak and change, celebrates sustained downward momentum loudly, answers sustained upward momentum with direct urgency and one practical next move, and varies its sentence structure as the saved pattern changes. It does not shame Lily, invent behavior, imply clinical credentials, recommend extreme restriction, or infer obesity or health status from pounds alone.

The current headline and final overlay point share the exact same value. Walk-forward errors, annual outcomes, base-model values, momentum diagnostics, and caps remain available to tests and diagnostics without becoming a caveat wall in the card. Full physiological modeling would require inputs such as diet and activity that Lily does not currently store.
